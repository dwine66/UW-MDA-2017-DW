...
country.plot <- function(dataset,njoin,nplot,maxval,pname){
  ...
  mapCountryData( sPDF, nameColumnToPlot=nplot,catMethod = seq(0,maxval,by = maxval/10),
                  ...
                  missingCountryCol = "grey50",oceanCol = "grey80")
}
# Basic ggplot barchart
gg.hbc.re.plot <- function(dataset,xvar,yvar,tname,xname,yname){
  ggplot(dataset, aes(x=reorder(xvar,-yvar),y=yvar)) +
    geom_bar(stat='identity') +
    theme(axis.text.x = element_text(angle = 90, hjust = 1)) + theme(aspect.ratio=2/3) +
    ggtitle(tname) + xlab(xname) +ylab(yname)
}
...
gtd.sub.fatal <- filter(gtd.sub,nkill!="NA",nkill>0)
#gtd.sub.fatal <- filter(gtd.sub,nkill>0)

#gtd.sub.fatal.US <- filter(gtd.sub,nkill!="NA")
gtd.sub.fatal.US <- filter(gtd.sub.fatal,country_txt=="United States")
...
# gg.hbc.re.plot(gtd.country.tt,country_txt,n,'Countries with the most terrorist attacks (1970-2015)',"Country","Number of Attacks")

# Map terrorist country of origin
...
par(mfcol=c(3,2),mai=c(0.1,0.1,0.25,0.1),omi=c(0.1,0,0.1,0))
...
# map European/Middle East attacks
par(mfcol=c(1,1), mai=c(0,0,.25,0), oma=c(0.25,0.25,0.25,0.25))
newmap <- getMap(resolution = "low")
...
# Domestic attacks on businesses
...
# Plot function doesn't work here either!
#gg.hbc.re.plot(dom.biz[1:10,],gname,n,'Top Ten terrorist groups attacking US Businesses, 1970-2015','Group','Number of Attacks')
...
# Illustrate US groups of unknown type
...
# Collect # of Fatalities and # events by country of origin
